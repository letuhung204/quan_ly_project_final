...
@Service
class TaskProgressServiceImpl implements TaskProgressService {
	@Autowired
	...
	/*******************************************************************************************************************
	 * Neu tao TaskProgress co dateCreate = X cho 1 task co task cha => ham se tao
	 * TaskProgress cho task cha, su dung so lieu lay tu cac TaskProgress duoc tao
	 * muon nhat truoc ngay X cua cac task con cua task cha do Neu khong co ban ghi
	 * TaskProgress nao cua task tuong ung co ngay muon hon ngay X thi se cap nhat
	 * progress cho task do
	 * 
	 * @param TaskProgress
	 */
	...
	public void createTaskProgress(TaskProgress taskProgress) {
		...
		if (task.getTaskIdparent() != null) {
			...
			if (taskParentProgress == null) {
				...
				taskParentProgress.setDetailLog(task.toString() + ": " + taskProgress.getDetailLog() + "\n");
				taskParentProgress.setConfirm(0);
			} else {
				String formerDetailLog = taskParentProgress.getDetailLog();
				taskParentProgress
						.setDetailLog(formerDetailLog + task.toString() + ": " + taskProgress.getDetailLog() + "\n");
			}
			long tRest;
			long tRestMax = 0L;
			...
			projectProgress = projectProgressService.findByProjectIdAndDateLog(task.getProjectId().getProjectId(),
					taskProgress.getDateLog());
			if (projectProgress == null) {
				...
				projectProgress.setConfirm(0);
				projectProgress.setDetailLog(task.toString() + ": " + taskProgress.getDetailLog() + "\n");
			} else {
				projectProgress.setDetailLog(
						projectProgress.getDetailLog() + task.toString() + ": " + taskProgress.getDetailLog() + "\n");
			}
			...
			for (Task bigTask : taskList) {
				...
				System.out.println("Task: " + bigTask.getTaskId() + " tRest: " + tRest / (3600L * 24L * 1000));
			}
			...
			int progressProject = 100 - (int) (100 * tRestMax
					/ (task.getProjectId().getDeadlineDate().getTime() - task.getProjectId().getStartDate().getTime()));
			if (progressProject < 0) {
				progressProject = 0;
			}
			Date lastProjectDate = projectProgressService
					.getLastDateOfProgjectProgress(task.getProjectId().getProjectId());
		...
		if (latestDate == null || (latestDate != null && latestDate.before(taskProgress.getDateLog()))) {
